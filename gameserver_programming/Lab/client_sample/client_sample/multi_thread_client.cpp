...
constexpr auto WINDOW_WIDTH = TILE_WIDTH * SCREEN_WIDTH + 10;   // size of window 스크린에서 보여주는 타일의 개수
...
int main()
{
	...
	sf::Socket::Status status = socket.connect("127.0.0.1", PORT_NUM); // 서버와 연결한다. 해당 소켓(socket)을 가지고 센드 리시브를 수행한다.
	...
	p.size = sizeof(p);
	p.type = CS_LOGIN;
	strcpy_s(p.name, "ohsd");
	send_packet(&p); // 패킷을 보낸다.
	...
	while (window.isOpen()) // 윈도우가 살아있다면..
	{
		sf::Event event;
